...
class PersonalInfo(BaseModel):
    ...
    name: str = ""
    label: str = Field("", description="Professional title (e.g., 'Principal Research Engineer')")
    email: str = ""
    phone: str = ""
    url: str = ""
    summary: str = ""
...
class WorkEntry(ResumeEntry):
    ...
    end_date: str = Field("", alias="endDate")
    summary: str = ""
    roles: list[str] = Field([], description="Role tags (e.g., 'ML/AI Expert', 'Tech Lead')")
    highlights: list[str] = []
    url: str = ""
    projects: list[Project] = Field([], description="Named projects within this position")
...
class Education(ResumeEntry):
    institution_id: EntryId = Field(description="Reference to institution")
    department: str = Field("", description="School or institute within the institution")
...
class Skill(BaseModel):
    ...
    name: str = Field(description="Subcategory name (e.g., 'Languages', 'ML Frameworks')")
    keywords: list[str] = []
...
class Certificate(ResumeEntry):
    ...
    format: str = Field("", description="Delivery format (e.g., 'In-Person', 'Remote', 'Online')")
...
class LeadershipCategory(BaseModel):
    ...
    name: str = Field(description="Subsection name (e.g., 'Mentoring & Team Leadership')")
...
class Interest(BaseModel):
    ...
    name: str
    description: str = Field("", description="Narrative description of the interest area")
    keywords: list[str] = []
...
class Meta(BaseModel):
    ...
    canonical: str = Field("", description="URL to the canonical version of this resume")
...
class Resume(BaseModel):
    ...
    quote: str = ""
    institutions: list[Institution] = Field([], description="Organizations referenced by work, education, certificates")
    work: list[WorkEntry] = []
    ...
    def all_entry_ids(self) -> set[EntryId]:
        ...
        for work in self.work:
            ...
            for project in work.projects:
                ids.add(project.id)
        for section in [
            self.education, self.publications, self.skills,
            self.languages, self.certificates, self.projects,
            self.patents, self.conferences, self.memberships,
        ]:
